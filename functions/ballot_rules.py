...
def check_fuzzy(ballots: list[Ballot], videos: dict[str, Video], similarity_threshold: float):
    ...
    for ballot in ballots:
        ...
        for property_name, similarity_set in similarity_sets.items():
            ...
            while len(unchecked_votes) > 0:
                ...
                similar_votes = set(
                    [
                        vote for vote in unchecked_votes
        ...
        for vote in ballot.votes:
            vote_similarity_properties = [prop for prop in property_order if vote in similarity_sets[prop]]
