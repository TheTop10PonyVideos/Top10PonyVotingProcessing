...
class CSVEditor(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.load_button = ttk.Button(self, text="Load processed CSV", command=self.load_processed_csv)
        ...
        self.canvas = tk.Canvas(self.data_frame, bd=0, xscrollcommand=self.scrollbar_x.set, yscrollcommand=self.scrollbar_y.set, width=1200, height=700)  
    ...
    def save_changes(self):
        for row_idx, row in enumerate(self.data):
            for col_idx, _ in enumerate(row):
                entry_widget = self.inner_frame.grid_slaves(row=row_idx, column=col_idx)[0]
