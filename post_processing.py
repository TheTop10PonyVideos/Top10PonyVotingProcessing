...
if len(sys.argv) < 3:
    print(f"python {sys.argv[0]} CALCULATED_TOP_10_CSV OUTPUT_FILE_NAME")
    print(
        """
Given a CSV file containing the result of calculating the top 10 pony videos
(with header line `Title,Percentage,Total Votes,URL`), output 3 files:
...
calculated_top_10_path_str = sys.argv[1]
output_file_name = sys.argv[2]
calculated_top_10_path = Path(calculated_top_10_path_str)

# Read CSV and extract URLs from the fourth column, skipping the first row
video_urls = []
with open(calculated_top_10_path, "r", newline="", encoding="utf8") as csvfile:
