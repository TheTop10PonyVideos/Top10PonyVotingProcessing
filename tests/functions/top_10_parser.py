...
from functions.top_10_parser import (
    group_records_by_headings,
    parse_calculated_top_10_csv,
)
...
class TestFunctionsTop10Parser(TestCase):
    ...
    def test_parse_calculated_top_10_csv(self):
        ...
        self.assertEqual(
            "Example 5", grouped_records["HISTORY"]["1 year ago"][0]["Title"]
        )
        self.assertEqual(1, len(grouped_records["HISTORY"]["5 years ago"]))
        self.assertEqual(
            "Example 6", grouped_records["HISTORY"]["5 years ago"][0]["Title"]
        )
        self.assertEqual(1, len(grouped_records["HISTORY"]["1 year ago"]))
        self.assertEqual(
            "Example 7", grouped_records["HISTORY"]["10 years ago"][0]["Title"]
        )
