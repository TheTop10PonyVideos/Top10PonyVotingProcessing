...
class YouTubeFetchService:
    ...
    def request(self, url: str):
        ...
        if video_id is None:
            raise ValueError(f'Could not request URL "{url}" via the YouTube Data API; unable to determine video id from URL')
        ...
        try:
            ...
            raise FetchRequestError(f'Could not request URL "{url}" via the YouTube Data API; error while executing request: {e}') from e

        if response is None:
            raise FetchRequestError(f'Could not request URL "{url}" via the YouTube Data API; no API response')
    ...
    def parse(self, response) -> dict:
        ...
        if not response["items"]:
            raise VideoUnavailableError(f'Unable to parse response from YouTube Data API; response does not contain any items')
...
class YtDlpFetchService:
    ...
    def request(self, url: str):
        ...
        try:
            ...
            raise FetchRequestError(f'Could not fetch URL "{url}" via yt-dlp; error while extracting video info: {e}') from e
